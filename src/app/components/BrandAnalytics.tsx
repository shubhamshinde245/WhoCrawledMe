...
import { TrendingUp, TrendingDown, Eye, MessageSquare, Share2, Heart, BarChart3, Target, Award, AlertTriangle, Zap, Users } from 'lucide-react';
...
export default function BrandAnalytics({ timeRange }: BrandAnalyticsProps) {
    ...
    const fetchBrandData = async () => {
      ...
      try {
        ...
        const mockBrandTrends: BrandTrend[] = [
          { date: '2024-01-01', visibility_score: 75.2, mention_rate: 198, sentiment_score: 76.4, engagement_rate: 5.2, share_of_voice: 19.8, brand_awareness: 58.3 },
          { date: '2024-01-02', visibility_score: 78.1, mention_rate: 212, sentiment_score: 77.8, engagement_rate: 5.6, share_of_voice: 20.4, brand_awareness: 59.7 },
          { date: '2024-01-03', visibility_score: 80.5, mention_rate: 225, sentiment_score: 79.2, engagement_rate: 6.1, share_of_voice: 21.2, brand_awareness: 61.2 },
          { date: '2024-01-04', visibility_score: 83.2, mention_rate: 238, sentiment_score: 78.6, engagement_rate: 6.4, share_of_voice: 22.1, brand_awareness: 62.8 },
          { date: '2024-01-05', visibility_score: 85.7, mention_rate: 251, sentiment_score: 78.1, engagement_rate: 6.7, share_of_voice: 22.8, brand_awareness: 63.9 },
          { date: '2024-01-06', visibility_score: 86.9, mention_rate: 243, sentiment_score: 78.5, engagement_rate: 6.9, share_of_voice: 23.2, brand_awareness: 64.1 },
          { date: '2024-01-07', visibility_score: 87.3, mention_rate: 234, sentiment_score: 78.9, engagement_rate: 6.8, share_of_voice: 23.4, brand_awareness: 64.2 }
        ];
        ...
        const mockCompetitorData: CompetitorComparison[] = [
          { brand: 'Your Brand', visibility: 87.3, mentions: 234, sentiment: 78.9, engagement: 6.8, market_share: 23.4, color: '#3b82f6' },
          { brand: 'Competitor A', visibility: 82.1, mentions: 198, sentiment: 75.2, engagement: 5.9, market_share: 19.8, color: '#ef4444' },
          { brand: 'Competitor B', visibility: 79.5, mentions: 176, sentiment: 73.6, engagement: 5.4, market_share: 17.2, color: '#10b981' },
          { brand: 'Competitor C', visibility: 76.8, mentions: 165, sentiment: 71.9, engagement: 5.1, market_share: 15.6, color: '#f59e0b' },
          { brand: 'Competitor D', visibility: 74.2, mentions: 142, sentiment: 69.3, engagement: 4.7, market_share: 13.8, color: '#8b5cf6' },
          { brand: 'Others', visibility: 65.4, mentions: 98, sentiment: 65.1, engagement: 3.9, market_share: 10.2, color: '#6b7280' }
        ];
        ...
        const mockTopMentions: TopMention[] = [
          {
            id: '1',
            platform: 'ChatGPT',
            content: 'WhoCrawledMe provides excellent AI platform monitoring with comprehensive analytics and real-time insights.',
            sentiment: 'positive',
            engagement: 156,
            reach: 2340,
          ...
          },
          {
            id: '2',
            platform: 'Claude',
            content: 'For tracking AI crawler activity, WhoCrawledMe offers detailed visibility into platform behavior and user interactions.',
            sentiment: 'positive',
            engagement: 134,
            reach: 1890,
          ...
          },
          {
            id: '3',
            platform: 'Perplexity',
            content: 'WhoCrawledMe dashboard shows comprehensive metrics but could improve real-time update frequency.',
            sentiment: 'neutral',
            engagement: 89,
            reach: 1456,
          ...
          },
          {
            id: '4',
            platform: 'Gemini',
            content: 'The competitive intelligence features in WhoCrawledMe provide valuable market insights for AI platform analysis.',
            sentiment: 'positive',
            engagement: 112,
            reach: 1678,
          ...
          },
          {
            id: '5',
            platform: 'ChatGPT',
            content: 'WhoCrawledMe interface needs better mobile optimization for monitoring on-the-go.',
            sentiment: 'negative',
            engagement: 67,
            reach: 923,
  ...
  const overallScore = brandMetrics.reduce((sum, metric) => sum + metric.value, 0) / brandMetrics.length;
  const positiveMetrics = brandMetrics.filter(metric => metric.trend === 'up').length;
  const totalMentions = brandTrends.reduce((sum, trend) => sum + trend.mention_rate, 0);
  const avgSentiment = sentimentData.reduce((sum, data) => sum + data.positive, 0) / sentimentData.length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-white/60 backdrop-blur-sm border-white/20">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-gray-700">Positive Trends</span>
            </div>
            <div className="text-2xl font-bold text-gray-900">{positiveMetrics}/{brandMetrics.length}</div>
        ...
        <Card className="bg-white/60 backdrop-blur-sm border-white/20">
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold text-gray-900">{totalMentions.toLocaleString()}</div>
        ...
        <Card className="bg-white/60 backdrop-blur-sm border-white/20">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-gray-700">Avg Sentiment</span>
            </div>
            <div className="text-2xl font-bold text-gray-900">{avgSentiment.toFixed(1)}%</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              {brandMetrics.map((metric) => (
                <Card key={metric.id} className="bg-white/60 backdrop-blur-sm border-white/20">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge 
                          variant="secondary" 
                          className={`text-xs ${
                            metric.category === 'visibility' ? 'bg-blue-100 text-blue-700' :
                            metric.category === 'engagement' ? 'bg-green-100 text-green-700' :
                            metric.category === 'sentiment' ? 'bg-purple-100 text-purple-700' :
                    ...
                    <div className="space-y-2">
                      <div className="flex items-end gap-2">
                        <span className="text-2xl font-bold text-gray-900">
                          {metric.name.includes('Rate') && !metric.name.includes('Mention') ? 
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${Math.min((metric.value / (metric.benchmark * 1.2)) * 100, 100)}%` 
                          }}
                        ></div>
                      </div>
                    </div>
              ...
              <CardContent>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart data={brandHealth} margin={{ top: 20, right: 30, bottom: 20, left: 30 }}>
        ...
        <TabsContent value="trends" className="space-y-6">
          ...
          <Card className="bg-white/60 backdrop-blur-sm border-white/20">
            ...
            <CardContent>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={brandTrends} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    <defs>
                ...
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={[
                          { name: 'Positive', value: sentimentData[sentimentData.length - 1]?.positive || 0, fill: '#10b981' },
                          { name: 'Neutral', value: sentimentData[sentimentData.length - 1]?.neutral || 0, fill: '#f59e0b' },
                          { name: 'Negative', value: sentimentData[sentimentData.length - 1]?.negative || 0, fill: '#ef4444' }
                        ]}
                        cx="50%"
                        cy="50%"
                        labelLine={false}
        ...
        <TabsContent value="competition" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {competitorData.map((competitor) => (
              <Card key={competitor.brand} className="bg-white/60 backdrop-blur-sm border-white/20">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold text-gray-900">{competitor.brand}</h3>
                    <div 
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{competitor.visibility.toFixed(1)}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{competitor.sentiment.toFixed(1)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Engagement:</span>
                      <span className="font-medium">{competitor.engagement.toFixed(1)}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Market Share:</span>
                      <span className="font-medium">{competitor.market_share.toFixed(1)}%</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
        ...
        <TabsContent value="mentions" className="space-y-6">
          <div className="space-y-4">
            {topMentions.map((mention) => (
              <Card key={mention.id} className="bg-white/60 backdrop-blur-sm border-white/20 hover:bg-white/70 transition-colors">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div className={`w-3 h-3 rounded-full ${
                        ...
                        'bg-red-500'
                      }`} title={`${mention.sentiment} sentiment`}></div>
                    </div>
