...
import { Search, TrendingUp, AlertCircle, Lightbulb, Target, Clock, Hash, Zap } from 'lucide-react';
...
export default function QueryDiscovery({ timeRange }: QueryDiscoveryProps) {
  ...
  const [sortBy, setSortBy] = useState<'frequency' | 'growth' | 'opportunity'>('frequency');

  useEffect(() => {
    const fetchQueryData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/query-discovery?timeRange=${timeRange}&type=overview`);
        ...
        const mockTrendingQueries: TrendingQuery[] = [
          {
            ...
            related_topics: ['content marketing', 'copywriting', 'social media'],
          ...
          },
          {
            ...
            related_topics: ['dashboard design', 'analytics', 'business intelligence'],
            first_seen: '2024-01-01',
          ...
          },
          {
            ...
            related_topics: ['programming languages', 'web development', 'learning'],
            first_seen: '2024-01-02',
          ...
          },
          {
            ...
            related_topics: ['threat intelligence', 'incident response', 'SIEM'],
        ...
        const mockContentGaps: ContentGap[] = [
          ...
          },
          {
            ...
            estimated_impact: 75,
            difficulty: 'medium',
            keywords: ['sustainable AI', 'green computing', 'environmental impact']
          },
          {
            id: '5',
        ...
        const mockTopicTrends: TopicTrend[] = [
          { date: '2024-01-01', ai_tools: 120, machine_learning: 95, automation: 78, data_science: 65, programming: 110 },
          { date: '2024-01-02', ai_tools: 135, machine_learning: 102, automation: 85, data_science: 72, programming: 118 },
          { date: '2024-01-03', ai_tools: 148, machine_learning: 108, automation: 92, data_science: 78, programming: 125 },
          { date: '2024-01-04', ai_tools: 162, machine_learning: 115, automation: 98, data_science: 84, programming: 132 },
          { date: '2024-01-05', ai_tools: 175, machine_learning: 122, automation: 105, data_science: 89, programming: 138 },
          { date: '2024-01-06', ai_tools: 189, machine_learning: 128, automation: 112, data_science: 95, programming: 145 },
          { date: '2024-01-07', ai_tools: 203, machine_learning: 135, automation: 118, data_science: 101, programming: 152 }
        ];

        // Mock query categories
        const mockQueryCategories: QueryCategory[] = [
          { name: 'AI Tools', count: 1247, growth: 34.2, color: '#3b82f6' },
          { name: 'Machine Learning', count: 892, growth: 28.7, color: '#10b981' },
  ...
  const filteredQueries = trendingQueries
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case 'opportunity': return b.opportunity_score - a.opportunity_score;
        default: return b.frequency - a.frequency;
      }
    });

  const totalQueries = trendingQueries.reduce((sum, query) => sum + query.frequency, 0);
  const avgGrowthRate = trendingQueries.reduce((sum, query) => sum + query.growth_rate, 0) / trendingQueries.length;
  const highOpportunityGaps = contentGaps.filter(gap => gap.demand_score >= 85).length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-white/60 backdrop-blur-sm border-white/20">
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold text-gray-900">{avgGrowthRate.toFixed(1)}%</div>
        ...
        <Card className="bg-white/60 backdrop-blur-sm border-white/20">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-gray-700">Content Gaps</span>
            </div>
            <div className="text-2xl font-bold text-gray-900">{contentGaps.length}</div>
            <div className="text-xs text-yellow-600">{highOpportunityGaps} high opportunity</div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="trending" className="space-y-6">
        ...
        <TabsContent value="trending" className="space-y-6">
          ...
          <div className="space-y-4">
            {filteredQueries.map((query, index) => (
              <Card key={query.id} className="bg-white/60 backdrop-blur-sm border-white/20 hover:bg-white/70 transition-colors">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-sm font-medium text-gray-500">#{index + 1}</span>
                        <h3 className="font-semibold text-gray-900">{query.query}</h3>
                        <Badge 
                          variant="secondary" 
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-600 mb-2">
                        <span>Frequency: <span className="font-medium text-gray-900">{query.frequency}</span></span>
                        <span>Growth: <span className={`font-medium ${
                          query.growth_rate >= 30 ? 'text-green-600' :
                          query.growth_rate >= 20 ? 'text-yellow-600' :
                          'text-red-600'
                        }`}>+{query.growth_rate.toFixed(1)}%</span></span>
                        <span>Opportunity: <span className="font-medium text-purple-600">{query.opportunity_score}/100</span></span>
                      </div>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className={`w-3 h-3 rounded-full ${
                        query.sentiment === 'positive' ? 'bg-green-500' :
                        query.sentiment === 'neutral' ? 'bg-yellow-500' :
                        'bg-red-500'
                      }`} title={`${query.sentiment} sentiment`}></div>
                      <span className="text-xs text-gray-500">{query.last_seen}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
        ...
        <TabsContent value="gaps" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {contentGaps.map((gap) => (
              <Card key={gap.id} className="bg-white/60 backdrop-blur-sm border-white/20">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-green-600">{gap.estimated_impact}/100</span>
                    </div>
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <Badge 
                        variant="outline" 
                        className={`text-xs ${
                          gap.difficulty === 'easy' ? 'border-green-500 text-green-600' :
                          gap.difficulty === 'medium' ? 'border-yellow-500 text-yellow-600' :
                    ...
                    <div>
                      <span className="text-xs text-gray-500 mb-1 block">Suggested Content:</span>
            ...
            <CardContent>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={topicTrends} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    />
                    <Line type="monotone" dataKey="ai_tools" stroke="#3b82f6" strokeWidth={2} name="AI Tools" />
                    <Line type="monotone" dataKey="machine_learning" stroke="#10b981" strokeWidth={2} name="Machine Learning" />
                    <Line type="monotone" dataKey="automation" stroke="#ef4444" strokeWidth={2} name="Automation" />
                    <Line type="monotone" dataKey="data_science" stroke="#8b5cf6" strokeWidth={2} name="Data Science" />
                    <Line type="monotone" dataKey="programming" stroke="#f59e0b" strokeWidth={2} name="Programming" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
        ...
        <TabsContent value="categories" className="space-y-6">
          <Card className="bg-white/60 backdrop-blur-sm border-white/20">
            ...
            <CardContent>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={queryCategories} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {queryCategories.map((category) => (
              <Card key={category.name} className="bg-white/60 backdrop-blur-sm border-white/20">
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={`font-medium ${
                        category.growth >= 30 ? 'text-green-600' :
                        category.growth >= 20 ? 'text-yellow-600' :
                        'text-red-600'
                      }`}>
