...
export default function ShareOfVoiceChart({
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // Fetch real share of voice data from API
        const response = await fetch(
          `/api/brand-analytics?timeRange=${timeRange}&type=competitors`
        );
        const result = await response.json();

        if (result.error) {
          console.error("Share of voice API error:", result.error);
          setShareData([]);
          setTrendData([]);
          return;
        }
  ...
  const topCompetitor = shareData
    .filter((item) => !item.isYourBrand)
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            ...
            <div
              className={`text-sm font-medium ${
                (yourBrand?.change || 0) >= 0
                  ? "text-green-600"
                  : "text-red-600"
              }`}
            >
              {(yourBrand?.change || 0) >= 0 ? "+" : ""}
              {yourBrand?.change.toFixed(1)}%
            </div>
          </div>
          ...
          <div className="bg-red-50 rounded-lg p-4 border border-red-200">
            ...
            <div className="text-xs text-red-700">{topCompetitor?.brand}</div>
          </div>
        </div>
