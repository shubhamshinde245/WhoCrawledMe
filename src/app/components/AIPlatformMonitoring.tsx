...
import { Bot, Activity, TrendingUp, AlertTriangle, CheckCircle, Clock, Brain, Zap } from 'lucide-react';
...
export default function AIPlatformMonitoring({ timeRange }: AIPlatformMonitoringProps) {
  const [platforms, setPlatforms] = useState<PlatformData[]>([]);
  const [categoryStats, setCategoryStats] = useState<CategoryStats[]>([]);
  const [trendData, setTrendData] = useState<TrendData[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedCategory, setSelectedCategory] = useState<string>('all');
  const [sortBy, setSortBy] = useState<'visits' | 'mentions' | 'engagement'>('visits');

  useEffect(() => {
    const fetchPlatformData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/ai-platforms?timeRange=${timeRange}&type=overview`);
        ...
        const mockCategoryStats: CategoryStats[] = [
          { category: 'Chatbots', count: 4, total_visits: 3318, avg_engagement: 88.5, color: '#3b82f6' },
          { category: 'Search', count: 2, total_visits: 857, avg_engagement: 83.5, color: '#10b981' },
          { category: 'Creative', count: 9, total_visits: 647, avg_engagement: 75.1, color: '#f59e0b' },
          { category: 'Research', count: 7, total_visits: 435, avg_engagement: 77.4, color: '#8b5cf6' },
          { category: 'Coding', count: 1, total_visits: 567, avg_engagement: 83.0, color: '#ef4444' }
        ];

        const mockTrendData: TrendData[] = [
          { date: '2024-01-01', chatgpt: 1100, claude: 750, gemini: 650, perplexity: 520, copilot: 480, total: 3500 },
          { date: '2024-01-02', chatgpt: 1150, claude: 780, gemini: 680, perplexity: 550, copilot: 490, total: 3650 },
          { date: '2024-01-03', chatgpt: 1200, claude: 820, gemini: 700, perplexity: 580, copilot: 510, total: 3810 },
          { date: '2024-01-04', chatgpt: 1180, claude: 850, gemini: 720, perplexity: 600, copilot: 520, total: 3870 },
          { date: '2024-01-05', chatgpt: 1220, claude: 870, gemini: 730, perplexity: 610, copilot: 540, total: 3970 },
          { date: '2024-01-06', chatgpt: 1240, claude: 885, gemini: 730, perplexity: 620, copilot: 560, total: 4035 },
          { date: '2024-01-07', chatgpt: 1247, claude: 892, gemini: 734, perplexity: 623, copilot: 567, total: 4063 }
        ];
  ...
  const totalVisits = platforms.reduce((sum, platform) => sum + platform.visits, 0);
  const activePlatforms = platforms.filter(p => p.status === 'active').length;
  const avgEngagement = platforms.reduce((sum, platform) => sum + platform.engagement_score, 0) / platforms.length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-white/60 backdrop-blur-sm border-white/20">
          <CardContent className="p-4">
            ...
            <div className="text-xs text-green-600">{activePlatforms} active</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/60 backdrop-blur-sm border-white/20">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-gray-700">Top Platform</span>
            </div>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredPlatforms.map((platform) => (
              <Card key={platform.id} className="bg-white/60 backdrop-blur-sm border-white/20 hover:bg-white/70 transition-colors">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <Badge 
                        variant="secondary" 
                        className={`text-xs ${
                          platform.category === 'chatbot' ? 'bg-blue-100 text-blue-700' :
                          platform.category === 'search' ? 'bg-green-100 text-green-700' :
                          platform.category === 'creative' ? 'bg-yellow-100 text-yellow-700' :
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-medium ${
                        ...
                        platform.status === 'inactive' ? 'text-yellow-600' :
                        'text-red-600'
                      }`}>
                        {platform.status}
                      </span>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{platform.visits.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{platform.engagement_score}/100</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Growth:</span>
                      <span className={`font-medium ${
                        platform.growth_rate >= 15 ? 'text-green-600' :
                        platform.growth_rate >= 10 ? 'text-yellow-600' :
                        'text-red-600'
                      }`}>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-xs">{platform.last_seen}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={trendData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    />
                    <Line type="monotone" dataKey="chatgpt" stroke="#3b82f6" strokeWidth={2} name="ChatGPT" />
                    <Line type="monotone" dataKey="claude" stroke="#ef4444" strokeWidth={2} name="Claude" />
                    ...
                    <Line type="monotone" dataKey="copilot" stroke="#8b5cf6" strokeWidth={2} name="Copilot" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
                    ...
                    <div key={category.category} className="p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <span className="ml-2 font-medium">{category.total_visits.toLocaleString()}</span>
                        </div>
                        <div>
                          <span className="text-gray-600">Avg Engagement:</span>
                          <span className="ml-2 font-medium">{category.avg_engagement.toFixed(1)}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${
                          platform.detection_confidence >= 90 ? 'bg-green-500' :
                          platform.detection_confidence >= 80 ? 'bg-yellow-500' :
                          'bg-red-500'
                        }`}></div>
                        <span className="text-sm font-medium">{platform.detection_confidence}%</span>
                      </div>
                    </div>
