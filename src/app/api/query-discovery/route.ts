...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (type) {
      ...
      case "trending":
        return await getTrendingQueries(category);
      case "gaps":
        return await getContentGaps();
      case "topics":
        return await getTopicTrends();
      case "categories":
        return await getQueryCategories();
      case "opportunities":
        return await getContentOpportunities();
      case "keywords":
        return await getKeywordAnalysis();
      default:
        return NextResponse.json(
          { error: "Invalid query discovery type" },
          { status: 400 }
        );
    }
  } catch (error) {
...
async function getTrendingQueries(category?: string | null) {
  try {
...
async function getQueryCategories() {
  try {
    const categories = [
      {
        name: "Technology",
    ...
    return NextResponse.json(
      { error: "Failed to fetch query categories" },
      { status: 500 }
    );
  }
}
...
async function getContentOpportunities() {
  try {
    const opportunities = [
      {
        ...
        description:
          "High search volume with low competition for comprehensive guides",
        opportunityScore: 94,
    ...
    return NextResponse.json(
      { error: "Failed to fetch content opportunities" },
      { status: 500 }
    );
  }
}
