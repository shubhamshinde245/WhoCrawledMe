...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (type) {
      case "overview":
        return await getCompetitiveOverview();
      case "benchmarks":
        return await getCompetitiveBenchmarks();
      case "market-share":
        return await getMarketShare();
      case "platform-comparison":
        return await getPlatformComparison();
      case "swot":
        return await getSWOTAnalysis();
      case "gaps":
        return await getCompetitiveGaps();
      case "positioning":
        return await getCompetitivePositioning();
      default:
        return NextResponse.json(
          { error: "Invalid competitive intelligence type" },
          { status: 400 }
        );
    }
  } catch (error) {
...
async function getCompetitiveOverview() {
  try {
    const overview = {
      marketPosition: 2,
      totalCompetitors: 12,
...
async function getCompetitiveBenchmarks() {
  try {
    const benchmarks = {
      categories: [
        {
    ...
    return NextResponse.json(
      { error: "Failed to fetch competitive benchmarks" },
      { status: 500 }
    );
  }
}
...
async function getSWOTAnalysis() {
  try {
    const swot = {
      strengths: [
...
async function getCompetitiveGaps() {
  try {
    const gaps = {
      featureGaps: [
        {
    ...
    return NextResponse.json(
      { error: "Failed to fetch competitive gaps" },
      { status: 500 }
    );
  }
}

async function getCompetitivePositioning() {
  try {
    const positioning = {
      quadrant: {
    ...
    return NextResponse.json(
      { error: "Failed to fetch competitive positioning" },
      { status: 500 }
    );
  }
}
