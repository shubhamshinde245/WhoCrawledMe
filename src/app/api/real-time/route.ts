    ...
    switch (type) {
      ...
      default:
        return NextResponse.json(
          { error: "Invalid real-time monitoring type" },
          { status: 400 }
        );
    }
  } catch (error) {
  ...
  try {
    const alerts = [
      ...
      },
      {
        ...
        description:
          "Average response time exceeded 3 seconds for ChatGPT requests",
...
async function getGeographicActivity() {
  try {
    const geographic = {
      ...
      heatmap: {
        data: [
          ...
          { lat: 40.7128, lng: -74.006, intensity: 0.9, city: "New York" },
          { lat: 51.5074, lng: -0.1278, intensity: 0.7, city: "London" },
          { lat: 52.52, lng: 13.405, intensity: 0.6, city: "Berlin" },
    ...
    return NextResponse.json(
      { error: "Failed to fetch geographic activity" },
      { status: 500 }
    );
  }
}
