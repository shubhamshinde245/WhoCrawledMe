...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (type) {
      case 'overview':
        return await getBrandOverview(startDate, brand);
      case 'visibility':
        return await getBrandVisibility(startDate, brand);
      case 'mentions':
        return await getBrandMentions(startDate, brand);
      case 'sentiment':
        return await getBrandSentiment(startDate, brand);
      case 'competitors':
        return await getCompetitorComparison(startDate, brand);
      case 'health':
        return await getBrandHealth(startDate, brand);
      case 'trends':
        return await getBrandTrends(startDate, brand);
      default:
...
async function getBrandOverview(startDate: Date, brand?: string | null) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch brand overview' }, { status: 500 });
  }
}

async function getBrandVisibility(startDate: Date, brand?: string | null) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch brand visibility' }, { status: 500 });
  }
}

async function getBrandMentions(startDate: Date, brand?: string | null) {
  try {
    const mentions = [
      {
        id: '1',
    ...
    return NextResponse.json({ error: 'Failed to fetch brand mentions' }, { status: 500 });
  }
}

async function getBrandSentiment(startDate: Date, brand?: string | null) {
  try {
    const sentimentData = {
      overall: 4.2,
      distribution: {
    ...
    return NextResponse.json({ error: 'Failed to fetch brand sentiment' }, { status: 500 });
  }
}

async function getCompetitorComparison(startDate: Date, brand?: string | null) {
  try {
    const competitors = [
      {
        name: 'Competitor A',
    ...
    return NextResponse.json({ error: 'Failed to fetch competitor comparison' }, { status: 500 });
  }
}

async function getBrandHealth(startDate: Date, brand?: string | null) {
  try {
    const healthMetrics = {
      overall: 92,
      metrics: {
    ...
    return NextResponse.json({ error: 'Failed to fetch brand health' }, { status: 500 });
  }
}

async function getBrandTrends(startDate: Date, brand?: string | null) {
  try {
    const trends = {
      keywords: [
    ...
    return NextResponse.json({ error: 'Failed to fetch brand trends' }, { status: 500 });
  }
}